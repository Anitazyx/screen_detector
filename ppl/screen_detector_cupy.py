...
os.environ["LD_LIBRARY_PATH"] = 'usr/local/cuda-10.0/lib64:$LD_LIBRARY_PATH'
os.environ["PATH"] = '/usr/local/cuda-10.0/bin:$PATH'
...
def find_color(src, color):
    ...
    mask_numpy = cv2.inRange(hsv, cp.asnumpy(color_dist['lower_'+color]), cp.asnumpy(color_dist['upper_'+color]))
    mask_cupy = cp.asarray(mask_numpy)
    return mask_cupy
...
# 定义一个测试函数
#计算柱状图的各个数值
def caculate_height(mask, color, value):
    ...
    if color=='green':#绿色的检测
        ...
        #     rectangle_info.append([x, h, color, 0])
        
        return mask_detect_new_cp(mask, 0, rectangle_info, color)
        ...
        sub_x_site = sub_x_site[0]
        # sub_x_site = cp.append(sub_x_site, [-1])
        ones = cp.ones(len(sub_x_site), dtype=np.int)
        
        
        sub_x_site = ones + sub_x_site
...
def wind_demo(wind, min_height_dist):
    ...
    for color, value in min_height_dist.items():
        ...
        if not is_all_black(find_color(wind_detect, color)):
            ...
            color_info = caculate_height(color_mask, color, value)
    ...
    if tag_unnormal == 1:
        ...
        unnormal_site = cp.where(cp.array(unnormal)>0)
        
        
        
        for i in iter(unnormal_site[0]):
            i = cp.asnumpy(i)
            print("异常公里标", tag_wind[i])
...
if __name__ == "__main__":
    ...
    color_dist = {'lower_blue':cp.array([115, 100,100]), 'upper_blue':cp.array([130, 255, 255]),
        'lower_dark_blue':cp.array([110, 100,100]), 'upper_dark_blue':cp.array([113, 255, 255]),
        'lower_green':cp.array([50, 100, 100]), 'upper_green':cp.array([70, 255, 255]),
        'lower_red':cp.array([0, 120, 120]), 'upper_red':cp.array([5, 255, 255]),
        'lower_orange':cp.array([15, 100, 100]), 'upper_orange':cp.array([25, 255, 255]), 
        'lower_yellow':cp.array([25, 100, 100]), 'upper_yellow':cp.array([35, 255, 255])
    }
    ...
    for fileName in files:
        ...
        time_detect_c = perf_counter()-start_c
        print(time_detect_c)
        print(time_detect)
