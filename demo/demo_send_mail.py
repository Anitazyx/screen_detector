...
def wind_demo(wind, min_height_dist):
    ...
    if tag_unnormal == 1:
        ...
        for i in range(len(rectangle_info)):
            ...
            print('   ')
        # wind_info[0].append("风速异常")
        
        # wind_info[1].extend(level)
        # wind_info[2].extend(tag)
        # wind_info[3].extend(result)
        # print(wind_info)
        return warn_info(type, level, tag, result)
    else:
...
def main(graph_list):
    ...
    message = warn_message(wind_info, rain_info, snow_info)
    return message


def send_mail(email_test, message, tag_info, level_info, src):
    
    tag_now = []
    level_now = []
    num = 0
    if message.wind_info is not None :
        num = 1
        tag_now.extend(message.wind_info.tag)
        level_now.extend(message.wind_info.level)
    if message.rain_info is not None :
        num = 1
        tag_now.extend(message.rain_info.tag)
        level_now.extend(message.rain_info.level)
    if  message.snow_info is not None:
        num = 1
        tag_now.extend(message.snow_info.tag)
        level_now.extend(message.snow_info.level)
    if num == 0:
        return None
  
    if tag_now==tag_info and level_now ==level_info:
        return None
    else:
        ...
        # print(now_time)
        email_test.send_message(message, now_time, src)
        
        return tag_now, level_now
...
if __name__ == "__main__":
    ...
    color_dist = {'lower_blue':np.array([115, 100,100]), 'upper_blue':np.array([130, 255, 255]),
        'lower_dark_blue':np.array([110, 100,100]), 'upper_dark_blue':np.array([113, 255, 255]),
    ...
    #定义三张柱状图的切分坐标
    crop_x = [420, 1700]
    crop_y = [170, 360, 490, 682, 775, 990]
    ...
    # message = warn_message(None,None, None)
   
    
    # message  = main(message)
    
    # message = multiprocessing.Manager().list()
    # p1=Process(target=main,args=(message))
    # p2=Process(target=send_mail,args=(message))
    ...
    #     if flag:
    #         print("_______________"+fileName+"_______________")
    ...
    #     print("time used:"+"%.3f" %time_detect+"s")

    tag_info = []
    level_info = []
    cap = cv2.VideoCapture("VE Project 3.avi")
    ...
    while 1:
        ...
        # p1 = Process(traget=main, args = (graph_list, message))
        # p2 = Process(traget=send_mail, args=(email_test, message, tag_info, level_info, src))
        # p1.start()
        # p2.start()
        ...
        message = main(graph_list)

        result = send_mail(email_test, message, tag_info, level_info, src)
        
        if result is not None:
            tag_info, level_info = result[0], result[1]
        else:
            continue
