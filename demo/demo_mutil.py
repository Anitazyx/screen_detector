...
def main(message, tag_mail, message_fifo):
    message_now = []
    ...
    while (cap.isOpened()):
        ...
        if [wind_info, rain_info, snow_info] != [None, None, None]:
            ...
            message_now = if_send_mail(message,message_now,tag_mail, message_fifo)
...
# def if_send_mail(message, message_now, tag_mail, message_fifo):
...
def if_send_mail(message, message_now, tag_mail, message_fifo):
    ...
    if message[len(message)-1] != message_now:
        message_now = message[len(message)-1]
        # message_fifo.extend([message[len(message)-1]])
...
def send_mail(tag_mail, message, message_fifo):
    ...
    while True:
        ...
        if tag_mail.value==1:
            ...
            # email_test.send_message_list(message)
# def send_mail(tag_mail,message_fifo):
...
#                 print("发送邮件")
#                 email_test.send_message_list(message_fifo[i])
#                 # time.sleep(random.random())
#                 del(message_fifo[i])
#             print(message_fifo)
...
if __name__ == "__main__":
    ...
    p1=Process(target=main,args=(message, tag_mail, message_fifo))
    
    p2=Process(target=send_mail,args=(tag_mail,message, message_fifo))
